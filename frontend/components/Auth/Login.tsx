...
import authlogo from "@/public/Assets/authlogo.jpg"

export function Login() {
  const router = useRouter()
  const [getRegister, { isLoading: registerLoading }] = useGetRegisterMutation();
  const [getLogIn, { isLoading: loginLoading }] = useGetLogInMutation();
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center bg-white p-8">
        <Card className="w-full max-w-md shadow-xl rounded-2xl">
          <CardHeader>
            ...
            <CardAction>
              <Button variant="link" onClick={() => { setIsSignup(!isSignup); setError({ username: "", email: "", password: "" }) }}>
              ...
              <div className="flex flex-col gap-6">
                {isSignup && (
                  <div className="grid gap-2">
                    ...
                    <Input id="username" type="text" name="username" value={login.username} placeholder="Enter your username" onChange={handleChange} />
                    {error.username && <span className="text-red-500 text-base">{error.username}</span>}
                  </div>
                )}
                <div className="grid gap-2">
                  <Label htmlFor="email">Email</Label>
                  <Input id="email" type="email" name="email" value={login.email} placeholder="m@example.com" onChange={handleChange} />
